...
if(data.response_type=="texto"){
  ...
  document.getElementById(`respuestasDesAg${surveyId}`).className="";
   document.getElementById(`editarCapituloRes${surveyId}`).className="";
   document.getElementById(`eliminarCapituloRes${surveyId}`).className="";
}else{
  fetch('/api/OptionQuestion')
  .then(response => response.json())
  .then(data => {
    ...
    data.forEach(OptionRespuesta => {
      ...
      document.getElementById(`respuestasDesAg${surveyId}`).className="bi-plus-square-fill";
      
   document.getElementById(`editarCapituloRes${surveyId}`).className="bi-pencil-fill";
   document.getElementById(`eliminarCapituloRes${surveyId}`).className="bi-trash3-fill";
    });
  })
  .catch(error => {
    console.error('Error:', error);
